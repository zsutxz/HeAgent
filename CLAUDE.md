...
- **沙箱会话生命周期（2026-08-26，FR-4）**：`SandboxSession` 提供同一 run 内 shell 命令的 cwd 跨命令保持（「cd 前缀 + 尾捕获」）与 run 结束 teardown（`sandbox_session_keep` 默认删除目录）；会话非安全边界（WinJob 无 FS 隔离、Firejail 非完美边界），crash 孤儿目录无 GC/保留策略（deferred），须 OS 级沙箱兜底。
