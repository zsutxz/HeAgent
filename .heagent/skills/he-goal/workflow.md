...
是把初步分析结论写回该 goal 的需求文档（文件名由 prompt 的 `Goal document` 行给出，见该步骤正文）。
Step 06 是**规划步骤**：把已验证范围拆成 story、排定工作项顺序、把每条 story 归入恰好一个 sprint，并固定每条
...
的步骤。需求文档的文件名以 prompt 里 `Goal document` 行为准（`Goal directory` 是它所在目录）：用
`file_read` 读该文件；若目录里的既有文件名与该行不同（存量 goal 沿用旧名），以该行为准写回同一份文档，
不得另建第二份。
然后按顺序收口：
...
3. 在最终回复里给出 `## 需求总结` 章节，内容与写入该需求文档的「总结的需求」一致。
