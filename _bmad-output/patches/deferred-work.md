...
- **`await proc.wait()` D-state 永久 hang [`src/heagent/tools/sandbox.py:59`] — pre-existing。** `_kill_and_reap` 的 `await proc.wait()` 在子进程处于不可中断内核态（Linux D-state / pipe transport 对端 hang）时不返回也不抛——`suppress(BaseException)` 只吞异常、对 hang 无能为力，整个 `except CancelledError` 块卡在 reap、裸 `raise` 永不执行 → CancelledError 被永久阻塞而非替换（取消信号丢失的另一种更坏形态）。本次 D-1 只覆盖「reap 抛异常」子集。**Severity:** LOW（触发极端：D-state 在本地命令执行罕见；pre-existing，改动前后一致）。**Suggested fix (future):** reap 用 `asyncio.wait_for(proc.wait(), timeout=N)` 包一层硬上界，超时即放弃 reap、仍上抛取消信号；或文档显式声明 reap 可阻塞。
- **`proc.kill()` 权限失败致子进程+pipe FD 泄漏 [`src/heagent/tools/sandbox.py:51-59`] — inherent / pre-existing。** `_kill_and_reap` 的 `proc.kill()` 抛非 `ProcessLookupError`（如 `PermissionError`，Windows `TerminateProcess` 对已退出进程亦可能抛 `ERROR_ACCESS_DENIED`）时逃出 `suppress(ProcessLookupError)`、`await proc.wait()` 不执行 → 子进程未杀未 reap、pipe transport 在 GC 前保持打开。**Severity:** LOW（inherent「杀不掉」限制，调用方不可修）。**Mitigation:** patch D-1-A 的 debug 日志已使该泄漏可诊断（运维可见 reap 失败线索）。**Suggested fix (future):** 即便 kill 失败仍尝试 `await proc.wait()`（kill 与 wait 解耦，kill 抛错不阻断 wait 回收 pipe FD）。
