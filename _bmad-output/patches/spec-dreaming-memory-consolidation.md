...
- **缓解（defense-in-depth，非真正边界）**：dreamer 工具白名单最小化（无 shell/file_write）、~~web 返回启发式标记~~（**未实现/deferred**——`web_fetch` 返回路径当前未接 `guard_content`，仅 MCP 工具经 `bridge_result`；见 `_bmad-output/patches/deferred-work.md`）、`dream_enabled` 默认关、`dream_max_iterations` 紧预算、`EventBus` 事件可审计。
