...
## mcp/ — MCP Client 集成周期（三阶段统一，2026-08-18 合并）

> 原三个目录 `mcp-client/`（Epic 11-13）、`mcp-v2-upgrade/`（Epic 14）、`mcp-client-v2/`（Epic 15-18）已于 2026-08-18 合并为 `mcp/`，核心文档按类型整合、执行产物原样保留。导航与编号对照见 `mcp/README.md`。
...
| `stories/` | V2 周期 story 文件（A.1~C.3，原内部编号 14-x/15-x/16-x） |
...
- **Epic 25–28（GUI）`done`**（12 stories，见 `baseline/sprint-status.yaml`；周期目录旧状态为规划期快照）。
