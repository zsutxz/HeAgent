...
inputDocuments:
  ...
  - _bmad-output/mcp-client/brief-decision-log.md
  - _bmad-output/mcp-client/prd.md
  - _bmad-output/mcp-client/prd-decision-log.md
  - _bmad-output/mcp-client/prd-review-rubric.md
...
> **作用域**：本文是 MCP Client 集成（新增 epic）的架构决策，不是 HeAgent 整体架构重写。既有架构基线见 `_bmad-output/baseline/architecture.md`；本文聚焦「如何把 MCP client 适配层干净地嵌入既有 DAG / `ToolRegistry` / `AgentLoop`」。
