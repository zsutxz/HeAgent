...
- **FR-1**：把 SDK pin floor 从 `mcp>=1.28,<2` 提到 `mcp>=1.28.1,<2`（对齐 v1 线最新 stable 1.28.1；现 pin 已排除 v2 alpha，bump floor 锁最新小档）。
...
- SDK pin `mcp>=1.28.1,<2`（v2 stable 后评估上界）。1.28.1 为 v1 线最新 stable（PyPI history，截至 2026-07-12）。
- **【订正 2026-07-12】** 原 research digest 误断「v1 最新 stable = 1.27.2 / 无 1.28.x」，rubric review F1 据此「CONFIRMED」亦误——实际 1.28.0 早在 2026-06-20 已被本仓库安装（mcp-client Story 1.1），1.28.1 为当前最新。故 mcp-client prd §6「v1.28.0 stable」**非笔误、正确**；FR-1/Story 14-1 已据此重定为「bump floor 到 1.28.1」（非降级到 1.27.2）。
...
> 已定稿：FR 编号独立空间 FR-1~5 + 全限定引用（见 §2 header note）。mcp-client prd §6「v1.28.0」原判笔误——**已订正：1.28.0 真实存在（2026-06-20 起本仓库已装），非笔误，无需改原文**（详见 §5 订正注）。
