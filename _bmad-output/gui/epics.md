...
4 个 Epic，24 个 FR，核心零改动。Epic 24 是硬前置，Epic 25 依赖 Epic 24，Epic 26/27 可在 Epic 25 进行中并行开发。

## FR Coverage Map

| FR | Epic | 主题 |
|----|------|------|
| FR-G1 | 24 | 启动 TUI + 主聊天界面 |
| FR-G2 | 24 | 流式 LLM 回复 |
| FR-G3 | 24 | Markdown 渲染 |
| FR-G4 | 24 | 用户/Agent 消息区分 |
| FR-G5 | 24 | 状态栏（模型/Token/迭代） |
| FR-G6 | 24 | 执行期间输入禁用 + 运行指示 |
| FR-G7 | 25 | 工具调用折叠卡片 |
| FR-G8 | 25 | 卡片展开（参数+结果） |
| FR-G9 | 25 | 工具失败红色标记 |
| FR-G10 | 25 | 工具执行耗时 |
| FR-G11 | 25 | `/model` 斜杠命令（含自动补全） |
| FR-G12 | 25 | `/mcp-prompt` 斜杠命令 |
| FR-G13 | 25 | `Ctrl+C` 中断 Agent |
| FR-G14 | 25 | `Ctrl+L` 清屏 |
| FR-G15 | 26 | 页面导航（F1-F6 快捷键） |
| FR-G16 | 26 | 技能管理表格（列表/详情/归档/删除） |
| FR-G17 | 26 | 技能创建弹窗 |
| FR-G18 | 26 | Cron 管理表格（增删） |
| FR-G19 | 26 | 记忆/画像查看 |
| FR-G20 | 27 | 运行历史树 |
| FR-G21 | 27 | 运行详情查看 |
| FR-G22 | 27 | 恢复运行 (resume) |
| FR-G23 | 27 | 事件日志实时流 |
| FR-G24 | 27 | 事件日志过滤 |
...
> 依赖 Epic 24（需要页面导航基础设施）；可与 Epic 25 并行。
...
> 依赖 Epic 24（Agent 运行基础设施）+ Epic 26（页面导航）；可与 Epic 25 并行。
...
| 25 | 3 | 2-3 天 | Epic 24 |
| 26 | 3 | 3-4 天 | Epic 24 |
| 27 | 3 | 2-3 天 | Epic 24 + 26 |
| **总计** | **12** | **9-13 天** | |

> Epic 26/27 可在 Epic 25 进行中并行开发（不同 Screen，零代码冲突）。
...
```
            ...
            ▼            ▼            ▼
       ┌──────────┐ ┌──────────┐ ┌──────────┐
       │ Epic 25  │ │ Epic 26  │ │ Epic 27  │
       │ 工具卡片 │ │ 管理面板 │ │ 可观测性 │
       │ 斜杠命令 │ │          │ │          │
       └──────────┘ └────┬─────┘ └────┬─────┘
                         └──────┬─────┘
                               │
                          (Epic 27 部分功能需 Epic 26 的页面导航)
```
