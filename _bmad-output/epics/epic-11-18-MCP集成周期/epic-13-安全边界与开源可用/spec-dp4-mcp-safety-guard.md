...
- `_bmad-output/sprint-status.yaml` — `action_items` DP-4 项 status `open → closed`（2026-09-15 注：该 `action_items` 块已从根 `sprint-status.yaml` 移除，条目为 `closed`）
...
- [x] `docs/frame.md` + `_bmad-output/sprint-status.yaml` -- 更新已知缺口 + action_items DP-4 关闭 -- 同步架构权威（2026-09-15 注：root `sprint-status.yaml` 的 `action_items` 块已移除）
