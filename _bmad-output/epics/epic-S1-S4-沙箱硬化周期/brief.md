...
preceded_by:
  - _bmad-output/epics/epic-01-10-主线规划周期/（epics 1-10，MVP + 自学习闭环）
  - _bmad-output/epics/epic-11-18-MCP集成周期/（阶段一，原 mcp-client/，epics 11-13，MCP Tools-only）
  - _bmad-output/epics/epic-11-18-MCP集成周期/（阶段三，原 mcp-client-v2/，epics 15-18，写操作治理 + Resources/Prompts + Git 工具）
created: 2026-07-20
updated: 2026-07-20
---
...
- ⬜ 沙箱执行的资源限额（`--rlimit-as` / `--rlimit-cpu`） — **已登记活动台账（2026-09-15）**：`sandbox.py` 无任何 rlimit 上界。
- ⬜ MCP server / cron 子进程接入沙箱 — **已登记活动台账（2026-09-15）**：MCP 一侧成立（`tools/mcp/manager.py:235` 的 stdio 子进程不经 `execute_in_sandbox`）；「cron 子进程」一侧不成立（`cron/` 无子进程路径）。
