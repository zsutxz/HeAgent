...
| Epic | FR | 主题 |
|------|----|------|
| Epic 25 | FR-G1~G6 | 流式聊天（最小可跑） |
| Epic 26 | FR-G7~G14 | 工具可视化 + 斜杠命令 + 中断 |
| Epic 27 | FR-G15~G19 | 管理面板（技能/Cron/记忆） |
| Epic 28 | FR-G20~G24 | 可观测性（运行历史/事件日志） |

## 四、Epic 依赖关系

```
Epic 25 (流式聊天) ──┬──▶ Epic 26 (工具可视化 + 斜杠命令)
                     │
                     └──▶ Epic 27 (管理面板) ──▶ Epic 28 (可观测性)
                              (Epic 27/28 可在 Epic 26 进行中并行开发)
```

- Epic 25 是所有后续 Epic 的**硬前置**——必须先有可跑的聊天界面
- Epic 26 依赖 Epic 25 的消息列表和输入框基础设施
- Epic 27 可与 Epic 26 并行（管理面板是独立 Screen，不依赖工具卡片）
- Epic 28 的部分功能（事件日志）可在 Epic 25 后随时接入，运行历史树需要 Epic 25 的 Agent 运行基础设施
